...
#######################################################################
# Get a list of units of selected variants via sliding windows
# based on basepairs
#
    ...
    win.start=0L, dup.rm=TRUE, verbose=TRUE)
{
    ...
    stopifnot(is.numeric(win.start), is.finite(win.start),
        length(win.start)==1L)
    ...
    for (chr in chrlst)
    {
        ...
        v <- .Call(SEQ_Unit_SlidingWindows, pos, idx, win.size, win.shift,
            win.start, dup.rm, integer(length(pos)))
