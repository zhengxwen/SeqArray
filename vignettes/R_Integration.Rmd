...
The SeqArray package is designed for R programming environment, and enables high-performance computing in the multi-core symmetric multiprocessing and/or loosely coupled computer cluster framework. The features of SeqArray are extended with existing R packages for WGS data analyses, and the R codes for demonstration are available in the package vignette `Integration with R`.


# Application Program Interface

We illustrate the SeqArray API by implementing an example to calculate the frequency of reference allele across chromosomes. If a genomic region is specified via `seqSetFilter()`, the calculation is performed over the region instead of all variants. `seqApply()` enables applying a user-defined function to the margin of genotypes, and the R code is shown as follows:
```{r}
library(SeqArray)

# open the SeqArray file in the package
...
The Rcpp package simplifies integration of compiled C++ code with R, and the function can be dynamically defined with inlined C/C++ codes:
```{r}
library(Rcpp)

cppFunction("double CalcAlleleFreq(IntegerVector x)
    {
        int n_x=x.size(), n=0, n0=0;
        for (int i=0; i < n_x; i++)
        {
            int g = x[i];
            if (g != NA_INTEGER)
...
```
where *IntegerVector* indicates the variable `x` is an integer vector and the function counts the numbers of zeros and non-missing values. The name *CalcAlleleFreq* can be passed to `seqApply()` directly.
```{r}
...
`seqParallel()` utilizes the facilities offered by the parallel package to perform calculations in parallel. The genotypes on a SeqArray file are automatically split into non-overlapping parts according to different variants or samples, and the results from client R processes are collected internally.
```{r}
af <- seqParallel(cl=4, file, FUN = function(f) {
    seqApply(f, "genotype", as.is="double", FUN=function(x) { mean(x==0L, na.rm=TRUE) })
}, split="by.variant")
head(af)
```
where the argument `cl=4` specifies the number of cores, the user-defined function `function(f) {...}` is distributed to 4 different computing nodes, and `split` indicates dividing genotypes according to different variants.



# Bioconductor Genomic Ranges

In this section, we illustrate how to work with Bioconductor core packages for performing common queries to retrieve data from a SeqArray file. The `GRanges` and `GRangesList` manipulate genomic range data and can be used in the function `seqSetFilter()` to define a data subset. For example, the annotation information of each exon, the coding range and transcript ID are stored in the `TxDb.Hsapiens.UCSC.hg19.knownGene` object for the UCSC known gene annotations on hg19.
```{r}
suppressPackageStartupMessages(library(TxDb.Hsapiens.UCSC.hg19.knownGene))

# get the exons grouped by gene
txs <- exonsBy(TxDb.Hsapiens.UCSC.hg19.knownGene, "gene")
```
where `exonsBy()` returns a `GRangesList` object for all known genes in the database.

```{r}
seqSetFilter(file, txs)  # define a filter

# VCF export with exon variants
seqGDS2VCF(file, "exons.vcf.gz")
```
If random-access memory is sufficiently large, users can load all exon variants via `seqGetData(file, "genotype")`; otherwise, data have to be loaded by chunk or a user-defined function is applied over variants by `seqApply()`.
