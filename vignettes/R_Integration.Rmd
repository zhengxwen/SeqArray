...
seqApply(f, "$dosage", function(x)
    {
        ...
        g <- (x - 2*p) / sqrt(p*(1-p))      # adjusted by allele frequency
...
# scaled by the trace over the number of samples
s <- s / (sum(diag(s)) / nrow(s))
...
# scaled by the trace over the number of samples
genmat <- genmat / (sum(diag(genmat)) / nrow(genmat))
...
The [GENESIS](http://www.bioconductor.org/packages/GENESIS) package offers methodology for estimating, inferring and accounting for population and pedigree structure in genetic analyses. The current implementation provides functions to perform PC-AiR and PC-Relate [@Conomos2015;  @Conomos2016]. PC-AiR performs PCA on whole-genome genotypes taking into account known or cryptic relatedness in the study sample. PC-Relate uses ancestry representative principal components to accurately estimate measures of recent genetic relatedness. In addition, the development version of GENESIS supports for SeqArray files in mixed model association testing and introduce aggregate tests of rare variants including burden and sequence kernel association tests [@Wu2011].
